...
// L298N
constexpr int pinIN1 = 4;
constexpr int pinIN2 = 18;
constexpr int pinIN3 = 21;
constexpr int pinIN4 = 22;

// Servos — GPIO25/26/27: pines limpios, sin funcion especial de boot
constexpr int servoBrazoIzqPin = 25;
constexpr int servoBrazoDerPin = 26;
constexpr int servoCabezaPin   = 27;
...
char mapUtf8Pair(uint8_t firstByte, uint8_t secondByte) {
  if (firstByte == 0xC3) {
    switch (secondByte) {
      case 0x81: return 'A'; case 0x89: return 'E'; case 0x8D: return 'I';
      case 0x91: return 'N'; case 0x93: return 'O'; case 0x9A: return 'U';
      case 0x9C: return 'U'; case 0xA1: return 'a'; case 0xA9: return 'e';
      case 0xAD: return 'i'; case 0xB1: return 'n'; case 0xB3: return 'o';
      case 0xBA: return 'u'; case 0xBC: return 'u'; default: return '?';
    }
  }
  if (firstByte == 0xC2) {
    switch (secondByte) {
      case 0xA1: return '!'; case 0xBF: return '?'; default: return '?';
    }
  }
  return '?';
}
...
String sanitizeForLcd(const String& value) {
  ...
  for (size_t i = 0; i < value.length(); ++i) {
    ...
    if (current >= 32 && current <= 126) { sanitized += static_cast<char>(current); continue; }
    if (current == '\t' || current == '\n' || current == '\r') { sanitized += ' '; continue; }
    if (i + 1 < value.length()) {
      sanitized += mapUtf8Pair(current, static_cast<uint8_t>(value[i + 1]));
      ++i; continue;
    }
    sanitized += '?';
  }
...
void clearScreen() {
  if (!lcdReady) return;
  for (int row = 0; row < lcdRows; ++row) writeLine(row, "");
}
...
void drawCenteredMessage(const String& message) {
  ...
  if (clean.length() <= lcdColumns) { writeCenteredLine(1, clean); return; }
...
// ═══════════════════════════════════════════════════════════
// Pantallas LCD por modo — deshabilitado
// ═══════════════════════════════════════════════════════════
...
void refreshMode1Screen() {
  ...
  for (int row = 0; row < lcdRows; ++row) writeLine(row, frame);
}
*/

// ═══════════════════════════════════════════════════════════
// Gestion de modos — deshabilitado
// ═══════════════════════════════════════════════════════════
...
void setMode(DisplayMode newMode) {
  ...
  scrollOffset = 0;
  lastScrollAt = 0;
  if (currentMode != DisplayMode::Mode2Telemetry) currentTelemetryFocus = TelemetryFocus::None;
  if (!lcdReady) return;
  switch (currentMode) {
    case DisplayMode::Standby:       drawStandbyScreen(); break;
    case DisplayMode::Driving:       drawDrivingScreen(); break;
    case DisplayMode::Mode1Input:    clearScreen(); refreshMode1Screen(); break;
...
void setScrollMessage(const String& newMessage) {
  ...
  scrollOffset = 0;
  lastScrollAt = 0;
  if (currentMode == DisplayMode::Mode1Input) refreshMode1Screen();
}
...
void setGreetingMessage(const String& input) {
  ...
  if (greetingPayload == lastGreetingPayload && millis() - lastGreetingSentAt < greetingRepeatGuardMs) return;
...
void setTelemetryValue(TelemetryFocus field, const String& newValue) {
  ...
  if (currentMode == DisplayMode::Mode2Telemetry && currentTelemetryFocus == field) renderTelemetryFocus();
}
*/
...
void processTelemetryButtons() {
  if (currentMode != DisplayMode::Mode2Telemetry) return;
  if (wasButtonPressed(hourButton))    { currentTelemetryFocus = TelemetryFocus::Hour;    renderTelemetryFocus(); }
  if (wasButtonPressed(weatherButton)) { currentTelemetryFocus = TelemetryFocus::Weather; renderTelemetryFocus(); }
  if (wasButtonPressed(nameButton))    { currentTelemetryFocus = TelemetryFocus::Name;    renderTelemetryFocus(); }
}
*/
...
void handleCommand(String rawCommand) {
  ...
  // ── Modos de pantalla — deshabilitado ─────────────────────
  ...
  // ── Control de ojos (LED) ─────────────────────────────────
  if (upperCommand == "ON")  { encenderOjos(); sendBluetoothLine("OK:ON");  return; }
  if (upperCommand == "OFF") { apagarOjos();   sendBluetoothLine("OK:OFF"); return; }

  // ── Mensajes / Saludos — deshabilitado ───────────────────
  ...
  // ── Limpieza — deshabilitado ──────────────────────────────
  // if (upperCommand == "CLEAR") { setScrollMessage(" "); return; }
...
void readBluetoothCommands() {
  while (SerialBT.available()) {
    ...
    // Fast-path: caracteres individuales sin prefijo de comando largo.
    // NOTA: B no esta en fast-path (colisiona con BRAZOS).
    //       D no esta en fast-path (colisiona con DRIVING).
    //       S no esta en fast-path (colisiona con STANDBY/SET_*).
    //       G no esta en fast-path (colisiona con GREETING:).
...
void setup() {
  ...
  // Escribir LOW en el registro antes de activar el driver — elimina el transitorio de boot
  digitalWrite(pinIN1, LOW);
  digitalWrite(pinIN2, LOW);
  digitalWrite(pinIN3, LOW);
  digitalWrite(pinIN4, LOW);
  ...
  Serial.println("Bluetooth listo. Comandos: F B L R S G H X D V P | ON OFF | BRAZOS CABEZA_IZQ CABEZA_CEN CABEZA_DER");
}
